...
APP_EXE_PATH = r"C:\sqldeveloper\sqldeveloper\bin\sqldeveloper64W.exe"  # -- Path to .exe file that you want to automate.
        ...
        while not (is_complete or failed_count >= 3) and dm.is_admin:
            with WinAppDriver(debug=True):
                try:
                    ...
                    desktopSession.find_element_by_xpath(f'//*[@AutomationId="{APP_EXE_PATH}"]').click()
                    # TopLevelWindow = desktopSession.find_element_by_xpath(f'//*[@Name="Oracle SQL Developer : Welcome Page"][@LocalizedControlType="window"]')
